...
7. **Data collection mode** — Ask only when at least one fetch will occur: always for modes 3–4; for modes 1–2 only if any relevant caches are stale or missing. See SKILL.md › Data Collection & Compilation.
...
4. Invoke the skill (if any) for the step with respect to the "Analysis & Output" rules in SKILL.md.
