...
| Historical financials | Income statement, cash flow, and balance sheet data — reuse Step 1 (Financial Model) output; no separate fetch required. **Fallback:** if Step 1 did not run, fetch these directly (see `data-fetch-protocol.md`). | _(from `*-financial-model.md`)_ | Inherits from Step 1 (1 year for historical data) |
